...
public class ObjectManager {
	...
	private BufferedImage[][] potionImgs,coinImgs, containerImgs;
...
//		coins = new ArrayList<>();
//		coins.add(new Coin(300,300,GOLD_COIN));
//		coins.add(new Coin(300,400,SILVER_COIN));
	...
	public void applyEffectToPlayer(Coin c) {
		if (c.getObjType() == GOLD_COIN)
			playing.getPlayer().changeScore(SILVER_COIN_VALUE);
		else if (c.getObjType() == SILVER_COIN)
			playing.getPlayer().changeScore(GOLD_COIN_VALUE);
	}
	...
	private void loadImgs() {
		...
		for (int j = 0; j < coinImgs.length; j++)
			for (int i = 0; i < coinImgs[j].length; i++)
				coinImgs[j][i] = coinSprite.getSubimage(12 * i, 16 * j, 12, 16);
	...
	private void drawCoins(Graphics g, int xLvlOffset) {
		for (Coin c : coins)
			if (c.isActive()) {
				int type = 0;
				if (c.getObjType() == SILVER_COIN)
					type = 1;
				g.drawImage(coinImgs[type][c.getAniIndex()], (int) (c.getHitbox().x - c.getxDrawOffset() - xLvlOffset), (int) (c.getHitbox().y - c.getyDrawOffset()), COIN_WIDTH, COIN_HEIGHT,
						null);
			}
	}
